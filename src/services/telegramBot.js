...
const processIncomingMessage = async ({
  ...
  if (!isWithinBusinessHours(config) && config.isAwayMessageEnabled && config.awayMessageText) {
    await sendReply({
      ctx,
      customerChatId,
      businessConnectionId,
  ...
  const canAutoReply = categoryAllowed && (isAutoMode || (isHybridMode && !aiResult.isEscalationRequired));
  const shouldDraft = allowDrafts && businessConnectionId && !canAutoReply;

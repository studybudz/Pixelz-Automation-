...
const generateSmartReply = async (incomingText, tone, customPrompt, customerName) => {
  try {
    ...
    return {
      category: parsed.category || 'custom_inquiry',
      suggestedReply: parsed.suggestedReply || 'Thanks for reaching out. A human agent will review this shortly.',
      confidenceScore: Number.isFinite(parsed.confidenceScore) ? parsed.confidenceScore : 0.5,
      isEscalationRequired: Boolean(parsed.isEscalationRequired)
    };
  } catch (error) {
